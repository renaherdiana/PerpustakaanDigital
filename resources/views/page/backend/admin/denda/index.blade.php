...
.filter-left{ display:flex; gap:15px; }
.filter-left input{ min-width:250px; padding:8px 12px; border-radius:10px; border:1px solid #e0e0e0; font-size:14px; }
...
.btn-action{ border:none; border-radius:8px; font-size:12px; padding:6px 10px; display:inline-flex; align-items:center; gap:4px; cursor:pointer; }
.btn-verify{ background:#dcfce7; color:#166534; }

.icon-btn{ width:36px; height:36px; border-radius:10px; display:flex; align-items:center; justify-content:center; font-size:17px; border:none; cursor:pointer; text-decoration:none; transition:0.2s; }
.icon-show{ background:#ecfdf5; color:#059669; }

.empty-data{ text-align:center; padding:40px; color:#888; font-size:14px; }
</style>
        ...
        <div class="table-responsive">
            <table class="table align-middle" id="dendaTable">
                ...
                <tbody>
                            ...
                            <td>
                                @if($status != 'selesai')
                                    <form action="{{ route('admin.denda.bayar',$item->id) }}" method="POST" style="display:inline;">
                                        @csrf
                                        <button type="submit" class="btn-action btn-verify">✔ Verifikasi</button>
                                    </form>
                                @else
                                    <a href="{{ route('admin.denda.show',$item->id) }}" class="icon-btn icon-show">
                                        <i class="bi bi-eye"></i>
                                    </a>
                                @endif
                            </td>
                    ...
                    @empty
                        <tr>
                            <td colspan="9" class="empty-data">Belum ada data denda</td>
                        </tr>
                    @endforelse
                </tbody>
            </table>
